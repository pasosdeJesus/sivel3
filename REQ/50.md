---
title: "R-#50: Citizen enrichment — simple text first, full form later"
labels: frontend, ux, database, api, mvp
---

## Step 1: Simple enrichment text (MVP now) ✅

Before building the full form, add a **simple text field** for the citizen to write improvements:

### Database
```sql
ALTER TABLE pre_alert ADD COLUMN citizen_notes TEXT;
```

### API: Modify `POST /api/pre-alerts/convert`
- Accept `citizen_notes` (optional, max 5000 chars)
- Store in `pre_alert.citizen_notes`
- Documenter sees this text when scoring

### Frontend: Replace "Convert" button flow
1. Button opens a simple inline form with:
   - Textarea: "Mejoras a la prealerta" (max 5000 chars)
   - "Submit" button → calls `POST /api/pre-alerts/convert`
2. After submit: "Awaiting documenter review" (current behavior)

### Documenter panel
- Show `citizen_notes` text in the scoring card so the documenter can read the citizen's improvements before scoring

### Acceptance Criteria
- [x] `citizen_notes` column added to `pre_alert`
- [x] Convert endpoint stores `citizen_notes`
- [x] Citizen sees textarea (max 5000 chars) in PreAlertModal
- [x] Documenter sees `citizen_notes` when scoring

---

## Step 2: Full enrichment form (future)

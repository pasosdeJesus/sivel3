...
**Auto-funding flow:** When a documenter scores 2-5, the backend automatically:

1. Determines the region from the pre-alert's `json_data.departamento`
2. Calls `withdraw(regionId, rewardAmount, rewardEscrowAddress)` on `SIVeL3RegionalDonationV2` to pull funds from regional donations
3. Calls `releasePayment(citizenWallet, rewardAmount)` on `SIVeL3RewardEscrow` to pay the citizen
4. Updates status to `paid`

**Why auto-withdraw from RegionalDonation:**
- Regional donations fund citizen rewards — same region, same cause
- No manual admin step required
- Transparent: donation → reward flow is on-chain and auditable
...
- [x] Contract verified on Celo Blockscout — deployed on Sepolia and Mainnet (`0xBFD94B391882612425455305dc0c9b1eC41E155A`)
...
- Deployed + verified on Sepolia and Mainnet (`0xBFD94B391882612425455305dc0c9b1eC41E155A`)
- Funded and wired to `POST /api/pre-alerts/:id/score` with `releasePayment()` on-chain
- EIP-191 signature verification on score endpoint prevents unauthorized releases
- `feedback` column added to `pre_alert` for documenter transparency

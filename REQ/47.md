...
- [ ] `DEFAULT_ADMIN_ROLE` correctly assigned to deployer.
- [ ] `BACKEND_ROLE` correctly assigned to backend wallet.
- [ ] `deposit()` transfers USDT from admin to contract.
- [ ] `releasePayment()` transfers USDT from contract to recipient (only `BACKEND_ROLE`).
- [ ] `releasePayment()` reverts if insufficient balance.
- [ ] `emergencyWithdraw()` allows admin to withdraw funds.
- [ ] Events emitted for all actions.
- [ ] Contract verified on Celo Blockscout.
- [ ] Funded from `SIVeL3RegionalDonationV2` (Colombia region) for testing.

# Vision: SIVeL 3 - A protocol for ethically witnessing political violence

## 1. Fundamental Principles

SIVeL 3 is governed by the following principles formulated by the priest Javier Giraldo

- **Maximum accessibility**Information on human rights violations belongs to the victims and to society. It must be free and open. The source code is licensed under ISC and the data under CC/BY. The general public and victims never pay for access to documented cases.
- **Confidentiality of information sources**\- for personal safety.
...
- **Citizens (whistleblowers)**– They browse the available pre-alerts, purchase access to a pre-alert for a small fee (discouraging spam), research the event using their own sources, enhance the pre-alert with verified information (photos, exact location, testimonials), and submit it as **citizen alert**. If verified, they receive a reward from the alerts and documenters fund (subject to availability). They can also send direct alerts without going through a pre-alert if they have firsthand information about a case not discovered by the AI agent.
...
- **AI Agent**– Independently monitors public sources (news, RSS, APIs) to detect potential human rights violations. Generates **pre-alerts** (structured reports in JSON following the SIVeL methodology and the conceptual framework of the Data Bank) and publishes them in a section of sivel.xyz

- **Documenters** They investigate citizen alerts, transform them into structured cases, and participate in peer review. They are neither full time employees of the validator. They receive a donation per documented case from the alerts and documenters fund ($5–$20 per case, depending on availability). They also help certify victims and family members when they file a claim. If the system achieves financial stability, they will receive a monthly stipend instead of payment per case. In Colombia, members of the Data Banks Network are already trained in the conceptual framework and simply need to register their wallet to receive the Documenter SBT and be preferred for authenticating victims in their region.

- **Regional Publication Validators** (e.g., CINEP's Political Violence Data Bank in Colombia) – A recognized human rights organization from the region that conducts the final audit for its region and certifies cases on the Celo blockchain. It maintains its own internal information system with full access to all data for its region. It also helps authenticate victims and their families when they file a claim.
...
- **Donors**– Donors donate USDT to document cases in specific regions or as micro-reparations to victims registered on the platform. They receive recognition and courtesy credits. If a donor is elected to a regional council for a term, they will have one vote (regardless of the amount donated).
...
2. **Detects potential events** using a small LLM running on hardware initially provided by the CINEP Data Bank.
3. **Generates pre-alerts**– structured reports in JSON following the SIVeL methodology and the conceptual framework of the Data Bank – and publishes them in the section for this purpose on sivel.xyz with blockchain certification.
...
| :---- | :---- | :---- |
| **CINEP Data bank** | 20% | Data access, conceptual framework, internal process |
| **Jesus' Steps (operator)** | 30% | Top of **$1,500 USD per month** Operation, development, hosting, AI agent, seed capital |
...
**Data license:** The cases used and contributed by AI agents and users are licensed **Creative Commons Attribution (CC/BY)** The copyright holder is the "CINEP Data Bank". This open license allows sivel.xyz to use, redistribute, and build upon the data, provided that proper attribution is given.
...
- When a region's Restoration Fund reaches **$5,000**, a **Regional Council** with victims/family members, active donors and citizen whistleblowers.
...
2. **Without ideological conditions**– Donations are accepted without political, Zionist or non-christian propaganda agendas.
...
The founding principles (see`PRINCIPLES.md`– including open-source licensing, non-commercialization of victim data, protection of sources, and alignment with IHL – cannot be altered by any governing body or donor. Pasos de Jesús retains permanent technical veto power to enforce them.
...
- **System integrity:** 1 Regional Validator and at least 3 Documenters incorporated with SBTs (of which at least 1 is from the Network of Data Banks in Colombia).
